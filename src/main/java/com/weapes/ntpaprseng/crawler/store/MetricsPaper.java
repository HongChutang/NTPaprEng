...
public class MetricsPaper implements Storable {
    ...
    public boolean store() {
        ...
        try {
            ...
            final String insert = "INSERT INTO REF_DATA(URL,Page_views, Web_of_Science,CrossRef, Scopus, News_outlets,reddit,Blog,Tweets,Facebook,Google,Pinterest,wikipedia,Mendeley,CiteUlik,Zotero,F1000,Video,linkedin,Q_A)" +
                    " VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?)";
            ...
            try (PreparedStatement preparedStatement = connection.prepareStatement(insert)) {
                ...
                // 判断执行是否成功
                succeed = preparedStatement.executeUpdate() != 0;
            }
            if (succeed) {
