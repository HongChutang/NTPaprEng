...
/**
 * Created by 不一样的天空 on 2016/10/16.
 */
public class UpdateLog {
    private static final String INSERTLOG =
            "INSERT INTO UpdateLog(" +
                    "URL," + "ArticlePosition," + "TotalNumber," + "UpdateTime)" +
                    "VALUES(?, ?, ?, ?)";
    private static final String UPDATELOG =
            "UPDATE UpdateLog SET " + "IsSuccessful = ?," + "SuccessfulNumber = ?," +
                    "FailedNumber = ?" + " WHERE URL = ";
    private static final String UPDATELOGTIME="UPDATE UpdateLog SET " + "AverageTime = ?"+
            " WHERE TotalNumber = ";
    public static void executeInsertLogSQL(String url, int articlePosition, int totalNumber, String updateTime) {
        ...
        try (final Connection connection = mysqlDataSource.getConnection()) {
            try {
                ...
                preparedStatement.setInt(2, articlePosition);
